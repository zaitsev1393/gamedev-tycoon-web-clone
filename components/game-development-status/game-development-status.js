...
export const createStatusBarComponent = (gameData) => {
  ...
  const bubblesContainer = createElement("div", {
    classes: ["bubbles-container"],
  });
  ...
  return {
    ...
    counters: {
      [POINT.TECHNICAL]: {
        update: (value) =>
          updateValue(
            techBubble.querySelector(`#${POINT.TECHNICAL}-value`),
            value
          ),
      },
      [POINT.DESIGN]: {
        update: (value) =>
          updateValue(
            designBubble.querySelector(`#${POINT.DESIGN}-value`),
            value
          ),
      },
      [POINT.BUG]: {
        update: (value) =>
          updateValue(bugBubble.querySelector(`#${POINT.BUG}-value`), value),
      },
    },
    nameContainer,
  };
